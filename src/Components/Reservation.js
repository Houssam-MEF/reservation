...
//     })
//     const {nom,prenom,tel,email,dateReserv} = useContext(reservationCont);
...
//                 <button className={page === formTitles.length - 1 ? "btn btn-outline-primary btn-lg" : "btn btn-outline-secondary btn-lg"} onClick={()=>{if (page === formTitles.length -1){alert('FORM SUBMITTED');console.log(formData);} else if (!nom.trim() || !prenom.trim() || !tel.trim() || !email.trim() || !dateReserv.trim()){
//                         return alert("toto")
//                     } else {
